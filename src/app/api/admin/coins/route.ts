...
import { addCoins, getCoins } from "@/system/lib/premium";
//==================
...
user.coinHistory.push({ type: "admin_set", amount: amount - old, reason: reason || "Admin set coins", timestamp: Date.now() });
...
const user = getUserByUsername(username);
if (!user) return NextResponse.json({ status: false, message: message.user.notFound }, { status: 404 });
const newCoins = { total: 0, earned: 0, spent: 0, lastUpdated: Date.now() };
updateUserCoins(username, newCoins, user.coinHistory || []);
