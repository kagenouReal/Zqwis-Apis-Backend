//vontoh syntax kode yang aku suka rapi nya
...
//==================
const mess = {
  owner: message.auth.owner,
  wait: message.status.processing,
  success: message.status.success,
  error: message.status.error,
  wrong: message.input.wrong,
};
//==================
const handler = async (conn: any, m: any) => {
  try {
    const body = m.body || "";
    const prefixes = (process.env.WA_PREFIX || ".,!,/").split(",");
    const prefix = prefixes.find((p: string) => body.startsWith(p)) || "";
    if (!prefix) return;
    const args = body.slice(prefix.length).trim().split(/ +/).slice(1);
    const command = body.slice(prefix.length).trim().split(/ +/)[0].toLowerCase();
    const text = args.join(" ");
    const botNumber = conn.decodeJid(conn.user.id);
    const ownerNumber = process.env.WA_GLOBAL_OWNER + `@s.whatsapp.net`;
    ...
    const admins = participants.filter((v: any) => v.admin !== null).map((v: any) => v.id);
    const isAdmins = m.isGroup ? admins.includes(m.sender) : false;
    const isBotAdmins = m.isGroup ? admins.includes(botNumber) : false;
