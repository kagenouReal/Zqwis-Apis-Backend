...
import { completeMission } from "@/system/lib/premium";
import { getUserByUsername } from "@/system/lib/account-db";

//==================
function getAvailableMissions(user: any) {
    ...
    return {
        daily: [
            { id: "daily_limit", name: "Daily API Limit", reward: dailyLimitAmount, rewardType: "limit" }, 
...
if (action === "list") {
    ...
    for (const cat of Object.keys(data)) {
        for (const m of data[cat]) {
            ...
            // If it's a daily mission and it's a new day, it's not completed yet
            m.isCompleted = isDaily ? (!isNewDay && completed.includes(m.id)) : completed.includes(m.id);
            m.canClaim = false;
            
            if (!m.isCompleted) {
                if (m.id === "daily_limit") {
                    m.canClaim = true; // Always can claim daily limit if not yet claimed today
                } else if (m.id === "daily_login") {
...
const res: any = await completeMission(token.name, missionId, mDetails.reward, mDetails.rewardType || 'coins');
if (!res.status) return NextResponse.json({ status: false, message: res.error }, { status: 400 });

return NextResponse.json({ 
    status: true, 
    message: message.mission.completed, 
    data: { missionId, reward: mDetails.reward, rewardType: mDetails.rewardType || 'coins' } 
});
} catch (e: any) {
return NextResponse.json({ status: false, message: e.message || message.api.serverError }, { status: 500 });
}
